...
from .audio import load_audio_16k_mono, preprocess_audio
...
def process_file(
    ...
    # 0) Normalize volume into a temp WAV used as the working source for both
    #    diarization and ASR. This boosts faint voices the ASR would otherwise
    #    miss; it preserves duration so timestamps stay aligned. The original
    #    path is kept only for naming the output. Cleaned up in `finally`.
    print(f"==> Volume normalization: {audio_path.name}")
    work_path = preprocess_audio(audio_path)
    try:
        # 1) Load audio as mono 16 kHz (for diarization/embeddings).
        print("==> Loading audio")
        samples = load_audio_16k_mono(work_path)
        ...
        if language:
            ...
            sentences = whisper_asr.transcribe(work_path, language=language)
        else:
            print("==> Transcription (Parakeet via MLX, auto language)...")
            model = asr.load_asr_model(model_name)
            sentences = asr.transcribe(model, work_path)
